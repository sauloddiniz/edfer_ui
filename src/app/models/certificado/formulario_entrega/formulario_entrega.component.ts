...
@Component({
    selector: 'app-formulario_entrega',
    templateUrl: './formulario_entrega.component.html',
    styleUrls: ['./formulario_entrega.component.css']
})

export class Formulario_entregaComponent implements OnInit {
        ...
        this.certificadoService.getClienteByNumeroPedido(value).subscribe(
            (clientes: any[]) => {
                ...
                for (const cliente of clientes) {
                    ...
                    const certificado =
                        cliente.produtoCertificadoAux.certificado;
            ...
            responseErr => {
                if (responseErr.status === 404) {
                    this.messageService.add({
                        severity: 'error',
                        summary: '404',
                        detail: 'Nº Pedido não encontrado'
                    });
                }
            }
        );
    ...
    public searchProduto(event: any) {
        this.certificadoService
            .getProdutoStartingWithCodigo(event.query)
            .subscribe(succes => {
